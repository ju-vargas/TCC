...
experiments = {
    'Baseline (BCD2)': {'load_sigma2': 0.0},
    'Load 1x sigma2 (BCD2)': {'load_sigma2': 1.0},
}
...
for mc in range(N_mc):
    ...
    for name, opts in experiments.items():
        ...
        W_bdac = bdac_mmse_reg(Hc, Rcc_inv, cfg)
        W_bcd1 = bcd_mmse_update_reg(W_bdac, Hc, Nc, cfg, Rcc_reg=Rcc)
        W_bcd2 = bcd_mmse_update_reg(W_bcd1, Hc, Nc, cfg, Rcc_reg=Rcc)
        
        S_det = detect(apply_equalizer(W_bcd2, Yc, cfg), const)
        e, t = count_errors(S_det, S_tx)
        results[name] += e
        if name == 'Baseline (BCD2)':
            tot += t

print(f"Results for BCD2 at 2 dB (Target ~0.000200):")
