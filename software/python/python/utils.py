...
def compute_local_cov(Nc, cfg: SimConfig, sigma2: float = 0.0):
    ...
    sigma2 : float, optional
        Background noise power for heuristic diagonal loading.
    ...
    for c in range(cfg.C):
        ...
        if cfg.diag_load_sigma2 > 0.0 and sigma2 > 0.0:
            R += cfg.diag_load_sigma2 * sigma2 * np.eye(cfg.Mc)
        Rcc.append(R)
