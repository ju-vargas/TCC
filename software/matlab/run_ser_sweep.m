...
for mc = 1:p.N_mc
    ...
    for isnr = 1:nSNR
        SNR_lin = 10^(p.SNR_dB(isnr)/10);
        % The figure x-axis is Eb/N0, so sigma2 = Es / (SNR_lin * bpS)
        sigma2 = p.Es / (SNR_lin * p.bpS);
        ...
        [Hc, ~, Nc] = partition_clusters(H_tgt, [], N_mat, p);

        % Local diagonal covariance blocks (with heuristic diagonal loading)
        Rcc = cell(p.C, 1);
        Rcc_inv = cell(p.C, 1);
        for c = 1:p.C
            R = (Nc{c} * Nc{c}') / p.N_samples;
            % Apply diagonal loading (diag_load_sigma2 = 1.0)
            R = R + 1.0 * sigma2 * eye(p.Mc);
            Rcc{c} = R;
            Rcc_inv{c} = inv_stable(R);
        end
