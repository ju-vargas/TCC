...
def sdr_mmse(Hc, Nc, Rcc_inv, Yc, cfg: SimConfig) -> np.ndarray:
...
def cdr_mmse(Hc, Nc, Rcc_inv, Yc, cfg: SimConfig) -> np.ndarray:
...
def bcd_mmse_update(W_old: list, Hc: list, Nc: list, cfg: SimConfig) -> list:
    """One Gauss-Seidel BCD sweep over all C clusters (Eq. 30).

    Parameters
    ----------
    W_old : list of (K, Mc) — current equalizer blocks
    Hc    : list of (Mc, K) — channel blocks
    Nc    : list of (Mc, N) — noise sample blocks
    ...
    # Initialise interaction variables (ring: sum over ALL clusters)
    ...
    for c in range(cfg.C):
        HcHc_T = Hc[c] @ Hc[c].conj().T       # Mc × Mc
        NcNc_T = (Nc[c] @ Nc[c].conj().T) / N  # Mc × Mc
        local_M = cfg.Es * HcHc_T + NcNc_T
        local_M_inv = inv_stable(local_M)

        # Remove old contribution of cluster c
